...
pic_download_dir = c.outputs_pictures_path + 'HuaBan/'  # 图片输出路径
...
# 画板输出文件
board_ids_file = c.outputs_logs_path + 'board_ids.txt'

# 图片key输出文件
pin_keys_file = c.outputs_logs_path + 'pin_keys.txt'
...
def catch_all_boards(user_url):
    ...
    for item in json_dict:
        coderpig.write_str_data(
            item['title'] + ':' + str(item['board_id']), board_ids_file)
    # 返回最后一个board_id
